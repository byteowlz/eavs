...
pub fn config_from_env(redirect_uri: String) -> Result<AnthropicOAuthConfig, String> {
    ...
    let scope = std::env::var("EAVS_OAUTH_ANTHROPIC_SCOPE").unwrap_or_else(|_| "claude:all".to_string());
...
pub fn build_authorize_url(config: &AnthropicOAuthConfig, state: &str) -> String {
    let mut url = url::Url::parse(AUTH_URL).unwrap_or_else(|_| url::Url::parse("https://console.anthropic.com/oauth/authorize").unwrap());
    url.query_pairs_mut()
